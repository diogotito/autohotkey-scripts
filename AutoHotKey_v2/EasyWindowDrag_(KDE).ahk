...
#LButton::
{
    ...
    ToolTip "Hotkey: " A_ThisHotkey "`nDoubleWin: " g_DoubleAlt
    if g_DoubleAlt
    {
        MouseGetPos ,, &KDE_id
        ; WinMinimize KDE_id ; quero depurar cenas primeiro
        info_text := (
            "Title`t`t"      WinGetTitle(KDE_id) "`n"
            "ID`t`t"         WinGetId(KDE_id) "`n"
            "Class`t`t"      WinGetClass(KDE_id) "`n"
            "Min/Max`t`t"    ((["Minimized", "neither", "Maximized"])
                               [2 + WinGetMinMax(KDE_id)]) "`n"
            "PID`t`t"        WinGetPid(KDE_id) "`n"
            "Process Name`t" WinGetProcessName(KDE_id) "`n"
            "Process Path`t" WinGetProcessPath(KDE_id) "`n"
            "[Ex] Style`t`t" Format("[{:x}] {:x}",
                                    WinGetExStyle(KDE_id),
                                    WinGetStyle(KDE_id))
        )
        A_Clipboard := info_text
        ToolTip info_text
        KeyWait "Alt"
        ToolTip()
        g_DoubleAlt := false
...
$LWin::
{
    global g_DoubleAlt := (A_PriorHotkey = "$LWin" and A_TimeSincePriorHotkey < 400)
    Send g_DoubleAlt ? "{LWin}" : "{LWin down}"  ; Dismiss Start menu
    Sleep 0
